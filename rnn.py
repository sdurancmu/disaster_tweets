...
class RnnClassifier(nn.Module):
    def __init__(self, device, params,embedding_matrix):
    ...
    def forward(self, inputs,hidden):
        ...
        if self.params.rnn_type == RnnType.GRU:
            final_state = hidden.view(self.params.num_layers, self.num_directions, batch_size, self.params.rnn_hidden_dim)[-1]
        elif self.params.rnn_type == RnnType.LSTM:
            final_state = hidden[0].view(self.params.num_layers, self.num_directions, batch_size, self.params.rnn_hidden_dim)[-1]
        # Handle directions
        ...
        log_probs = self.sig(X)
        log_probs = log_probs.view(batch_size, -1)
        log_probs = log_probs[:, -1] # get last batch of labels
